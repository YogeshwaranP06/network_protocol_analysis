...
**Sequence observed:**
1. DNS query resolved `github.com` → NAT64 IPv6 address mapping real IPv4 `20.207.73.82`
2. TCP 3-way handshake: SYN → SYN-ACK → ACK on port 443
3. TLS **Client Hello** sent immediately after TCP handshake
4. TLS **Server Hello** + Certificate + Change Cipher Spec returned
5. Client Change Cipher Spec → first encrypted Application Data (page load begins)
...
**Sequence observed:**
1. DNS query for `www.youtube.com`, followed by a separate lookup for the CDN hostname — YouTube separates the main site from its video-serving CDN
2. Immediately after CDN resolution, a **1292-byte UDP packet** was sent directly to port 443 — no TCP SYN at any point
3. Decoded as a **QUIC Initial packet**, confirmed via header byte `0xC0` (long header, Initial type)
4. 26 total UDP packets exchanged with this single CDN endpoint, most near the 1292-byte QUIC max datagram size — continuous encrypted video delivery
...
📄 Full report (Word format): [`Network_Protocol_Analysis_TLS_vs_QUIC.docx`](./Network_Protocol_Analysis_TLS_vs_QUIC.docx)
